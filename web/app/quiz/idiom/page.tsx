...
export default function IdiomQuizPage() {
  ...
  const [questions, setQuestions] = useState<QuizQuestion[]>(() =>
    generateQuizQuestions(getAllIdioms(), "idiom-to-meaning", questionCount)
  );
  ...
  const handleRestart = () => {
    setQuestions(generateQuizQuestions(getAllIdioms(), "idiom-to-meaning", questionCount));
    setCurrentIndex(0);
    setScore(0);
    setIsComplete(false);
  };
  ...
  return (
    <div>
      <div className="mb-3 flex justify-between items-center">
        <h2 className="text-base sm:text-lg font-semibold text-gray-800">성어 → 뜻 맞추기</h2>
        <span className="text-sm text-purple-500 font-medium">{score}점</span>
      </div>
